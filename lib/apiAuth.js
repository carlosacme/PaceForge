...
/** JWT legacy (anon/service_role). Las keys nuevas `sb_*` no son JWT. */
function isLegacyJwtApiKey(key) {
  return typeof key === "string" && key.trim().startsWith("eyJ");
}

/**
 * Headers admin para /rest/v1.
 * PostgREST toma el rol de `Authorization`, no de `apikey`. Sin JWT en Bearer
 * la query corre como `anon`, RLS oculta `athletes` y canAccessAthlete da 403.
 * Las secret nuevas (`sb_secret_`) no pueden ir en Bearer (Invalid JWT).
 */
export function adminHeaders(extra = {}) {
  ...
  if (isLegacyJwtApiKey(SERVICE_KEY)) {
...
export async function deviceTokensByUserId(userId) {
  ...
  try {
    const rows = await sbGet(
      `device_tokens?user_id=eq.${encodeURIComponent(userId)}&select=id,token,platform,last_seen_at&order=last_seen_at.desc`
    );
...
export async function deviceTokensByUserIds(userIds) {
  ...
  try {
    const rows = await sbGet(
      `device_tokens?user_id=in.(${ids.map(encodeURIComponent).join(",")})&select=id,token,platform,user_id,last_seen_at`
    );
    const byUser = {};
