...
import { androidChatNotificationTag } from "./chatNotificationTag.js";
...
export async function pushTargets(userId) {
  ...
  if (Array.isArray(rows) && rows.length) {
    return filterStaleDeviceTokens(rows).map((r) => ({ token: r.token, platform: r.platform || null }));
  }
...
export async function sendFCM(token, title, body, data) {
  ...
  // Chat: tag UNICO por mensaje (`chat:<party>:<uniq>`). Sin tag, FCM pone
  // `FCM-Notification:<n>` y no podemos filtrar la bandeja. Un tag fijo
  // (el de antes) sustituye la anterior. Entrenos/racha no llevan tag.
  const chatTag = androidChatNotificationTag(data);
  message.android = {
    priority: "high",
    notification: {
