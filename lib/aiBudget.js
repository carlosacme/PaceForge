...
/**
 * 403 si no es coach/admin (salvo `{ requireCoach: false }`, p.ej. briefing
 * del atleta). 429 si ya está en el tope (sin incrementar).
 * Si cabe, incrementa el cupo diario y el mensual, y deja pasar.
 *
 * @param {{ requireCoach?: boolean }} [options]
 */
export async function assertGenerateBudget(res, user, options = {}) {
  ...
  if (requireCoach && !canGenerateWorkouts(profile)) {
