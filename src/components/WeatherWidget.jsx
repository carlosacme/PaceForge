...
const INTENSITY_STYLES = {
  normal: { bg: "rgba(34,197,94,.1)", border: "rgba(34,197,94,.4)", color: "#166534", icon: "🌤️" },
  caution: { bg: "rgba(245,158,11,.1)", border: "rgba(245,158,11,.4)", color: "#92400e", icon: "⚠️" },
  ...
  useEffect(() => {
    ...
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => fetchWeather(null, null), // fallback a defaultCity
        { timeout: 5000 }
      );
    } else {
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8, flexWrap: "wrap" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: ".72em", color: st.color, marginTop: 2 }}>
            Sensación {weather.feelsLike}°C · {weather.humidity}% humedad · 💨 {weather.windSpeed} km/h
          </div>
        </div>
      </div>
